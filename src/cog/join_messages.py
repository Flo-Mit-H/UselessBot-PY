...
class JoinMessages(commands.Cog):
    ...
    async def on_member_join(self, member):
        ...
        await main.message.send_json(channel=channel, json_data=self.join_message["join-message"], msg=message)
        if self.join_message["join-dm"] != "":
            ...
            await main.message.send_json(channel=member, json_data=self.join_message["join-dm"], msg=dm)
    ...
    async def on_member_remove(self, member):
        ...
        await main.message.send_json(channel, self.join_message["leave-message"], msg=message)
