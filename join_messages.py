...
class JoinMessages(commands.Cog):
    ...
    async def on_member_join(self, member):
        ...
        message = main.replace_relevant(self.join_message["join-message"], member.guild)
        message = main.replace_member(message, member)
        await channel.send(message)
        if self.join_message["join-dm"] != "":
            dm = main.replace_relevant(self.join_message["join-dm"], member.guild)
    ...
    async def on_member_remove(self, member):
        ...
        message = main.replace_relevant(self.join_message["leave-message"], member.guild)
